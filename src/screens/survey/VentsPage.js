...
export default function VentsDetailsPage() {
  ...
  const nextPressed = async () => {
    const { type, condition, isWeatherResistant, isLockable, isVegitationFree, isStable, isFloodingFree, isExplosionReliefRoof, height, width, length, isAccessible, isSteps } = ventDetails;
  
    if (!type) {
      EcomHelper.showInfoMessage("vent Type is required. Please enter the vent Type.");
    } else if (!condition) {
      EcomHelper.showInfoMessage("vent Condition is required. Please enter the vent Condition.");
    } else if (isWeatherResistant === null) {
      EcomHelper.showInfoMessage("Is vent weather resistant? Please select an option.");
    } else if (isLockable === null) {
      EcomHelper.showInfoMessage("Is vent lockable? Please select an option.");
    } else if (isVegitationFree === null) {
      EcomHelper.showInfoMessage("Is vent free of vegetation, trees, etc.? Please select an option.");
    } else if (isStable === null) {
      EcomHelper.showInfoMessage("Is vent/housing stable? Please select an option.");
    } else if (isFloodingFree === null) {
      EcomHelper.showInfoMessage("Is vent free of flooding? Please select an option.");
    } else if (isExplosionReliefRoof === null) {
      EcomHelper.showInfoMessage("Is vent roof an explosion relief roof? Please select an option.");
    } else if (!height) {
      EcomHelper.showInfoMessage("vent height is required. Please enter the vent height.");
    } else if (!width) {
      EcomHelper.showInfoMessage("vent width is required. Please enter the vent width.");
    } else if (!length) {
      EcomHelper.showInfoMessage("vent length is required. Please enter the vent length.");
    } else if (isAccessible === null) {
      EcomHelper.showInfoMessage("Is vent easily accessible? Please select an option.");
    } else if (isSteps === null) {
      EcomHelper.showInfoMessage("Are there steps leading up to the vent? Please select an option.");
    } else {
