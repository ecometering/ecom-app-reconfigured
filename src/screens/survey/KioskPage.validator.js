...
export const validateKioskDetails = (kioskDetails) => {
  ...
  // First, validate if kiosk exists
  const kioskExistsError = validateBooleanField(
    'Kiosk Exists',
    kioskDetails.kioskExists,
    'Please indicate whether a kiosk exists.'
  );

  if (kioskExistsError) {
    errors.push(kioskExistsError);
  }

  // Only proceed with other validations if kioskExists is true
  if (kioskDetails.kioskExists) {
    const validations = [
      () =>
        validateRequiredField(
          'Kiosk Type',
