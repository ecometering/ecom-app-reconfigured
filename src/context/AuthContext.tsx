...
const login =async (username:string,password:string) => {
    ...
    console.log("file : AuthContext.tsx ~ line 31 ~ login ~ result", result);
        setAuthState({token: result.data.refresh
        ,authenticated:true});
        console.log("file : AuthContext.tsx ~ line 34 ~ login ~ authState", authState);

        axios.defaults.headers.common["Authorization"] = `Bearer ${result.data.refresh}`;
        ...
        return { error: true, msg: e?.response?.data?.msg || "An unknown error occurred" };
...
const logout = async () => { 
    ...
    console.log ("file : AuthContext.tsx ~ line 50 ~ logout ~ token removed", authState.token);
    setAuthState({token:null,authenticated:false});
    console.log ("file : AuthContext.tsx ~ line 52 ~ logout ~ authState", authState);
    console.log("file : AuthContext.tsx ~ line 53 ~ logout ~ token removed", authState.token);
};
