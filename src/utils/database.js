...
async function createJobsTable(db) {
  ...
  db.transaction((tx) => {
    tx.executeSql(
      ...
      (_, error) =>
        console.error('[createJobsTable] Error creating Jobs table:', error)
    );
  });
}
...
async function getDatabaseJob(setJob, jobStatus = undefined) {
  try {
    const db = await openDatabase(); // Open the database
...
const fetchModelsForManufacturer = async (meterType, manufacturer) => {
  console.log(
    `[fetchModelsForManufacturer] Start fetching models. MeterType: ${meterType}, Manufacturer: ${manufacturer}`
  );
  ...
  if (!tableName) {
    console.error(
      `[fetchModelsForManufacturer] Invalid meter type provided: ${meterType}`
    );
    throw new Error('Invalid meter type');
  }
  console.log(
    `[fetchModelsForManufacturer] Using table: ${tableName} for queries.`
  );
  ...
  try {
    ...
    const models = await new Promise((resolve, reject) => {
      db.transaction(
        (tx) => {
          ...
          console.log(
            `[fetchModelsForManufacturer] Executing query: ${query} with Manufacturer: ${manufacturer}`
          );
          ...
          tx.executeSql(
            ...
            [manufacturer],
            (_, result) => {
              const modelsArray = result.rows._array.map((row) => ({
                label: row['Model Code (A0083)'],
                value: row['Model Code (A0083)'],
              }));
              console.log(
                `[fetchModelsForManufacturer] Models fetched successfully. Count: ${modelsArray.length}`
              );
              resolve(modelsArray);
            },
            (_, error) => {
              console.error(
                `[fetchModelsForManufacturer] Error fetching models for Manufacturer: ${manufacturer}`,
                error
              );
              reject(error);
            }
        ...
        () => {
          ...
          db.close(
            () =>
              console.log(
                '[fetchModelsForManufacturer] Database closed after fetching models.'
              ),
            (error) =>
              console.log(
                '[fetchModelsForManufacturer] Error closing database:',
                error
              )
          );
        }
    ...
    console.error(
      `[fetchModelsForManufacturer] Failed to fetch models for MeterType: ${meterType} and Manufacturer: ${manufacturer}:`,
      error
    );
  ...
  db.transaction((tx) => {
    tx.executeSql(
      ...
      (_, { rows }) => {
        console.log(
          `Schema of ${tableName}:`,
          JSON.stringify(rows._array, null, 2)
        );
      },
      (_, error) => {
  ...
  return new Promise((resolve, reject) => {
    db.transaction((tx) => {
      tx.executeSql(
        ...
        [jobId],
        (_, { rows }) => {
          const existingPhotosJSON =
            rows.length > 0 ? rows._array[0].photos : JSON.stringify([]);
