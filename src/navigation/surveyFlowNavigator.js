...
const setAndSeal = (meterType, meterPressure) => {
  ...
  return meterType === 'Diaphragm'
    ? 'ExistingRegulator'
    : 'StreamsSetSealDetails';
};

const nextAfterMeterPhoto = ({
  corrector,
  datalogger,
  meterType,
  meterPressure,
...
const generateScreenInstancesForStreams = (numberOfStreams) => {
  ...
  for (let i = 0; i < numberOfStreams; i++) {
    ...
    const streamScreens = [
      ...
      {
        name: `SealedSlamShutPhotoPage-${i}`,
        component: GenericPhotoPage,
        title: `Sealed Slam Shut Photo ${i + 1}`,
        photoKey: `sealedSlamShutPhoto-${i}`,
      },
      {
  ...
  return (
    <Stack.Navigator>
      ...
      <Stack.Screen
        ...
        initialParams={{ title: 'Existing Streams' }}
      />
